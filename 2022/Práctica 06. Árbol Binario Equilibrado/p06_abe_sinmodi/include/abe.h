...
bool Abe<Key>::FindR(const Key &data, Node<Key> *current) const {
  ...
  if (*current == data) //  Comprobando si los datos ya están en el árbol. Si es así, devuelve verdadero.
    return true;
  return FindR(data, current->left) || FindR(data, current->right); // Comprobando si los datos están en el subárbol izquierdo o derecho.
}
...
template <class Key> bool Abe<Key>::Insert(const Key &data) {
  ...
  if (this->Find(data)) //  Comprobando si los datos ya están en el árbol. Si lo es, devuelve falso.
    return false;

  return InsertR(this->root_, data); // Llamando a la función recursiva InsertR.
}
