...
public:
  ...
  virtual bool Find(const Key &data) const;
...
template <class Key> bool Ab<Key>::Find(const Key &data) const {
...
template <class Key>
void Ab<Key>::PrintInorder(std::ostream &os) const {
...
// Llamando a la función recursiva PrintR.
template <class Key> void Ab<Key>::Print(std::ostream &os) const {
  PrintR(root_, os);
}

// Imprimiendo el árbol en un orden de nivel.
template <class Key> void Ab<Key>::PrintR(Node<Key> *prev, std::ostream &os) const {
