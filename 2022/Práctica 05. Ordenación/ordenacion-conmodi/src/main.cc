...
int main() {
  std::vector<Vehiculo> vec = {{"1"}, {"4"}, {"2"}, {"3"}};
  std::vector<Vehiculo> solucion;

  Algorithm<Vehiculo>* algoritmo = new insercion<Vehiculo>;

  solucion = algoritmo->metodo(vec, vec.size());
  // Mostrar solucion
  std::cout << std::endl;
  std::cout << "Solucion: "<< std::endl;
  for (auto& vehiculo : solucion) // instanciamos
    std::cout << vehiculo; // solucion final 
  std::cout << std::endl;
