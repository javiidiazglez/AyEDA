...
class shellsort : public Algorithm<Clave> {
 public:
...
template <class Clave>
std::vector<Clave> shellsort<Clave>::metodo(std::vector<Clave> a, int sz) {
  this->vector_ = a;
  shell_sz = sz;
  float alpha = 0;
  ...
  if (alpha <= 0 || alpha >= 1) {
    ...
    return shellsort<Clave>::metodo(a, sz);
  }
...
template <class Clave>
void shellsort<Clave>::swap_pos(int i, int j) {
  if (i != j) {
