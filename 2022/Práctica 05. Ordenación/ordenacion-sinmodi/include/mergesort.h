...
class mergesort : public Algorithm<Clave> {
 public:
 ...
 private:
  std::vector<Clave> auxilar;
  int merge_sz;
};

template <class Clave>
std::vector<Clave> mergesort<Clave>::metodo(std::vector<Clave> a, int sz) {
  // contador_global = 0;
  this->vector_ = a;
  merge_sz = sz;
...
template <class Clave>
void mergesort<Clave>::merge_sort(int inic, int fin) {
  int centro, i, j, k;
  if (inic < fin) {
    centro = (inic + fin) / 2;
...
template <class Clave>
void mergesort<Clave>::show_merge_changes(int i) {
  // contador_swap++;
  std::cout << std::endl << "< ";
