...
//    assert(i > 0 && i <= get_row() && j > 0 && j <= get_col());
//    return grid_[i][j]; // dejamos i j ya que 0 es lin invis
// }
...
void Grid::imprimirGrid(){            
  for(int i = 0; i <= row_ + 1; i++){    // comienzo por 1 
    for(int k = 0; k <= col_ +1; k++){
        cout << grid_[i][k];          //se mete en la celul
    }
    cout << endl;
  }
}
  ...
  if (row_ != 0 || col_ != 0){
    ...
    for (int i = 0; i < row_ + 2; i++){
        ...
        for (int h = 0; h < col_ + 2; h++){
          grid_[i][h] = Cell(i,h);
          if (i == 0 || h == 0 || i == row_ +1 || h == col_ +1){
            grid_[i][h].setState(neutral);
          }
