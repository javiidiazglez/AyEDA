...
void World::CheckLimits(int row, int column) {    // comprueba una fila o columna no existe (añade fila o columna)
  if (row < min_rows_) { // < -1 (hacia la izquierda)
    world_map_.AddRowFront(abs(min_rows_ - row), COLOR);    // crea una columna hacia la izquierda
    min_rows_ = row;
  } else if (row >= max_rows_) {
    world_map_.AddRowBack(abs((max_rows_ - row) + 1), COLOR);   // expande a la derecha
    max_rows_ += abs((max_rows_ - row) + 1);
  }

  if (column < min_columns_) {
    world_map_.AddColumnFront(abs(min_columns_ - column), COLOR);   // expande hacia arriba
    min_columns_ = column;
  } else if (column >= max_columns_) {
    world_map_.AddColumnBack(abs((max_columns_ - column) + 1), COLOR);  // expande hacia abajo
